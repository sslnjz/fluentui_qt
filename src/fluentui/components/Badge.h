...
public:
  ...
  enum class Appearance { filled, ghost, outline, tint };
  Q_ENUM(Appearance)

  enum class PresetSize { tiny, extrasmall, small, medium, large, extralarge };
  Q_ENUM(PresetSize)

  enum class Shape { circular, rounded, square };
  Q_ENUM(Shape)

  explicit Badge(QWidget *parent = nullptr, Appearance appearance = Appearance::filled, 
    Color c = Color::brand, Shape s = Shape::circular);
  ~Badge() override;
